...
struct control control;     /* storing controller info */
pthread_rwlock_t lock;      /* lock for the key_stats hash table */
...
static void send_report(int fd, short events, void *ptr) {
    int report_hit = 0;
    int report_key = 0;
    struct evbuffer *output = bufferevent_get_output(control.rep_ev);
    if (pthread_rwlock_wrlock(&lock) != 0) {
        fprintf(stderr,"send_report: can't acquire write lock\n");
        exit(-1);
    }
    if (key_stats == NULL) {
    ...
    if (HASH_COUNT(key_stats) == 0) {
        pthread_rwlock_unlock(&lock);
        return;
    }
    KEY_HIT *record;
    for (record = key_stats; record != NULL; record = record->hh.next) {
        if (record->hit > 0) {
            report_key ++;
            report_hit += record->hit;
            evbuffer_add_printf(output, "1:%s:%d\r\n", record->kname, record->hit);
            record->hit = 0; /* every time only report delta */
        }
    }
    pthread_rwlock_unlock(&lock);
    if (report_key > 0) {
        printf("Reported %d keys with total %d hits\n", report_key, report_hit);
    }
}
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("controller_ip", "%s", 
            settings.controller_ip ? settings.controller_ip : "NULL");
    APPEND_STAT("controller_port", "%d", settings.controller_port);
    APPEND_STAT("report_interval", "%d", settings.report_interval);
}
    ...
    do {
        while(key_token->length != 0) {
            ...
            if (it) {
                ...
                if (settings.controller_ip != NULL) {
                    if (pthread_rwlock_wrlock(&lock) != 0) {
                    ...
                    char *key = ITEM_key(it);
                    KEY_HIT *key_hit;
                    HASH_FIND_STR(key_stats, key, key_hit);
                    if (key_hit == NULL) {
                        key_hit = malloc(sizeof(KEY_HIT));
                        key_hit->hit = 0;
                        strcpy(key_hit->kname, key);
                        HASH_ADD_STR(key_stats, kname, key_hit);
                    }
                    key_hit->hit += 1;
                    pthread_rwlock_unlock(&lock);
                }
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'o': /* It's sub-opts time! */
            ...
            break;
        case 'x': /* IPv4 address string for remote controller */
            settings.controller_ip = optarg;
            printf("Set controller IP %s\n", settings.controller_ip);
            break;
        case 'y': /* Port */
            settings.controller_port = atoi(optarg);
            printf("Set controller port %d\n", settings.controller_port);
            break;
        case 'z': /* Interval in secs */
    ...
    /* Connect remote controller if specified */
    if (settings.controller_ip != NULL) {
        control.controller_addr.sin_family = AF_INET;
        control.controller_addr.sin_port = htons(settings.controller_port);
        inet_aton(settings.controller_ip, &control.controller_addr.sin_addr);
        printf("Attempt to connect the remote controller %s\n",
           settings.controller_ip);
        control.rep_ev = bufferevent_socket_new(
                         main_base, -1, BEV_OPT_CLOSE_ON_FREE);
        bufferevent_setcb(control.rep_ev, NULL, NULL, eventcb,
                      &control.controller_addr);
        if (bufferevent_socket_connect(control.rep_ev,
                                   (struct sockaddr *) &control.controller_addr,
                                   sizeof(control.controller_addr)) < 0) {
            /* Error starting connection */
            bufferevent_free(control.rep_ev);
            control.rep_ev = NULL;
            return -1;
        }
    }
